...
/** GLOBAL VARIABLES      */
/**************************/
std::string gWindowName = std::string("Get Training Data ObjRec");
size_t gSaveCalls = 0;
cv::Mat gRgbImg;
cv::Mat gPclMap;
...
void save( int state, void* userdata ) {
  ...
  for( size_t i = 0; i < gBoundingBoxes.size(); ++i ) {
    // Create subimage from the bounding box
    cv::Mat img( gRgbImg, cv::Rect( gBoundingBoxes[i](0),
				    gBoundingBoxes[i](1),
				    gBoundingBoxes[i](2)-gBoundingBoxes[i](0),
				    gBoundingBoxes[i](3)-gBoundingBoxes[i](1)) );
    // Store
    char imageName[100];
    sprintf( imageName, "cropped_%d_%d.png", gSaveCalls, i );
