...
OpenNI2Capture::OpenNICapture (const std::string& device_id)
  : grabber_ (device_id)
  , most_recent_frame_ ()
  , frame_counter_ (0)
  , use_trigger_ (false)
  , trigger_ (false)
{
  ...
  boost::function<void (const PointCloudConstPtr&)> frame_cb = boost::bind (&OpenNI2Capture::onNewFrame, this, _1);
  // ... and start grabbing frames
  grabber_.registerCallback (frame_cb);
  grabber_.start ();
}

OpenNI2Capture::~OpenNICapture ()
{
...
void
OpenNI2Capture::setTriggerMode (bool use_trigger)
{
  ...
  if (use_trigger_)
  {
    if (!preview_)
    {
      ...
      boost::function<void (const pcl::visualization::KeyboardEvent&)> keyboard_cb =
        boost::bind (&OpenNI2Capture::onKeyboardEvent, this, _1);
...
const PointCloudPtr
OpenNI2Capture::snapAndSave (const std::string & filename)
{
...
void
OpenNI2Capture::onNewFrame (const PointCloudConstPtr &cloud)
{
  mutex_.lock ();
  ++frame_counter_;
...
void
OpenNI2Capture::onKeyboardEvent (const pcl::visualization::KeyboardEvent & event)
{
