...
% Normalized x,y,z
xc = (nx*x + ny*y + nz*z - px*nx - py*ny - pz*nz);
...
Ji = [ diff(Fi,a), diff(Fi,b), diff(Fi,c), diff(Fi,e1), diff(Fi,e2), diff(Fi,px), diff(Fi,py), diff(Fi,pz), diff(Fi,ra), diff(Fi,pa), diff(Fi,ya), diff(Fi,k) ];
...
J6 = [ diff(F6,a), diff(F6,b), diff(F6,c), diff(F6,e1), diff(F6,e2), diff(F6,px), diff(F6,py), diff(F6,pz), diff(F6,ra), diff(F6,pa), diff(F6,ya), diff(F6,k) ];
...
ccode(Fi,'file','Fi_t.txt');
ccode(Ji,'file','Ji_t.txt');
...
ccode(F6,'file','F6_t.txt');
ccode(J6,'file','J6_t.txt');


ccode(Er_g,'file','Er_g_t.txt');
ccode(Er_d,'file','Er_d_t.txt');
ccode(Er_r,'file','Er_r_t.txt');
