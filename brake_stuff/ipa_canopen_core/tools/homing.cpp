...
int main(int argc, char *argv[]) {
	...
	if (!canopen::openConnection(deviceFile,canopen::baudRate)){
		...
		if (!canopen::motor_fault[CANid])
		{
			...
			if (canopen::mdeg2rad(pos) <= (home_offsets[CANid]+tolerance) && canopen::mdeg2rad(pos) >= (home_offsets[CANid]-tolerance))  home = true;
			if (canopen::mdeg2rad(pos) - home_offsets[CANid] < 0) negative = false;
			else negative = true;
