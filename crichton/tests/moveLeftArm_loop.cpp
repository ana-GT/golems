...
int main( int argc, char* argv[] ) {
	...
	if( poll(&stdin_poll, 1, 0 ) == 1 ) {
		...
		qtarget = q[arm_drive_ind] + dv; int trials = 0; int maxTrials = 105;
		while( q[arm_drive_ind] != qtarget && trials < maxTrials ) {
		 
		  for( int j = 0; j < ARM_AXES; ++j ) { dq[j] = 0; printf( " %f ", q[j]); } printf("\n");
		  dq[arm_drive_ind] = dv;
		  control_n( ARM_AXES, dq, 0.01, &chan_ref );	
		  update(); 
		  trials++;
		}
		printf("Done with trials \n");
